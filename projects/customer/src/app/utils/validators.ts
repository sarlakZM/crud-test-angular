...
export function uniqueMultiFieldValidator(existingValues: any[], dialogData: { [key: string]: any }): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    try {
      ...
      const isUnique = !values.find(
        (item) =>
          item.firstname == firstname && item.lastname == lastname && item.dateOfBirth == dateOfBirth.toDateString()
      );
      return isUnique
        ? null
        : {
            uniqueMultiField: {
              value:
                'Customers must be unique: These values ( First Name, Last Name and Date Of Birth) are already taken.',
            },
          };
    } catch (e) {
      return { invalidMobileNumber: { value: e } };
